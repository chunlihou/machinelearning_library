...
2. ?? Feature scaling (depend)
3. ?? Dimensionality reduction (depend) 
	  1. Feature selection (regression model)
	  2. Feature extraction (PCA, LDA, Kernel PCA)
4. ## Model fitting
5. #? Prediction 
	  1. Realtime search - no)
6. #? Evaluation
      1. Supervised learning - regression (RMSE)
	  2. Supervised learning - classification (accuracy)
	  3. Unsupervised learning (no need)
7. ## Visulization
8. #$ Model selection (due to the model difference, they need to be stablised to be compared)

# Package loading
```{r loadPackages}
pacman::p_load(dplyr, broom, caTools, ggplot2, gridExtra, lattice)
```
...
glimpse(dataset)
```
1. Chosing specificly columns (dataset = dataset[, c(?) or -c(?)]).

### Taking care of missing data (unnecessary)
```{r}
...
### Encoding categorical data (unnecessary)
```{r}
summary(dataset)
...
3. Only the dependent variable does not need the scale; the rest of all variables need to set under the same scale.
4. Scaling should be ahead of dataset partition.
...
# Regression
1. Regression for predicting a continuous data.
...
# Error rate evaluating
library('forecast')
accuracy(y.pred$.fitted, y.pred$Salary)
...
# Classification
1. Classification for predicting a categorical data.
2. Supervised learning, having a label variable or dependent variable or correct answer.
...
# Grid search applying for the best parameter
library('caret')
